# Olx Printed CLI Agent Guide

This directory is a generated `olx-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
olx-pp-cli doctor --json
olx-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
olx-pp-cli which "<capability>" --json
olx-pp-cli <command> --help
```
...
```bash
olx-pp-cli <command> --help
olx-pp-cli <command> --dry-run --agent
```
