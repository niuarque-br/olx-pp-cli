...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "olx-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("OLX_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("OLX_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "olx-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/olx-pp-cli/feedback.jsonl.
When ` + "`OLX_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`OLX_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  olx-pp-cli feedback list
  olx-pp-cli feedback list --limit 5
  olx-pp-cli feedback list --json`,
